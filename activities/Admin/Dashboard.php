...
class Dashboard extends Admin{
	...
	public function index(){
        ...
        FROM posts LEFT JOIN comments ON posts.id = comments.post_id GROUP BY posts.id ORDER BY comment_count DESC LIMIT 0,5")->fetchAll();

        $lastComments = $db->select("SELECT comments.id , comments.comment , comments.status , users.username
