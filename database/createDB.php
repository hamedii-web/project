...
class createDB extends DataBase {
	...
	private $tableInitilizes = [
		'table' => 'users' , 'fields' => [
			'username', 'email', 'password', 'permission'
		] , 'values' => [
			'hamedjoon' , 'hamed@gmail.com' , '1234567' , 'admin'
		]
	];
	public function run(){
		foreach ($this->createTableQueries as $create_table_query){
			$this->create($create_table_query);
		}

		foreach ($this->tableInitilizes as $tableInitialize){
